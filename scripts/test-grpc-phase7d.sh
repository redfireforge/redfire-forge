...
DELIVERABLES=(
  ...
  src-tauri/src/grpc/stream/mod.rs
  src-tauri/src/grpc/stream/helpers.rs
...
if [[ "${RUST_TEST_COUNT:-0}" -lt 13 ]]; then
  echo "Expected at least 13 Phase 7D Rust unit tests, found ${RUST_TEST_COUNT:-0}" >&2
  exit 1
fi
...
cd src-tauri
cargo test -p redfireforge -- grpc::stream_registry 2>&1
cargo test -p redfireforge -- grpc::stream:: 2>&1
echo "✓ Rust stream module tests passed"
cd ..
echo ""
