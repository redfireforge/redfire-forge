...
### Changed
- **Learning Hub / TLS demos — certificates renewed (expire 2036).** Kafka, GraphQL, and WebSocket demo TLS certs were regenerated (3650-day validity). **Install this app version**, then restart any local TLS Docker stacks (`docker compose down && docker compose up -d`). Restarting Docker by itself does not update certificates inside an older Learning Hub build. Kafka TLS and WebSocket TLS lessons paste CA/client material from the app; those steps will fail against the new stacks until you update. GraphQL TLS is unaffected if you paste `ca.crt` from disk.
