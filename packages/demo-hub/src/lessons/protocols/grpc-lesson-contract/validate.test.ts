...
describe('validateGrpcLessonRoster', () => {
  it('validates all 16 canonical roster entries', () => {
    const result = validateGrpcLessonRoster();
    expect(result.ok, result.issues.map((i) => `${i.path}: ${i.message}`).join('\n')).toBe(true);
    expect(GRPC_LESSON_ROSTER).toHaveLength(16);
  });

  it('has unique ids and sequential numbers 1–16', () => {
  ...
  it('matches the published GRPC-1–16 lesson id roster from the plan', () => {
