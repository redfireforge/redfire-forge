...
import { WebSocketComposePane, type WebSocketComposePaneProps } from './WebSocketComposePane';
import type { WsMessageTemplate } from '../../shared/websocket/types';

function defaultProps(overrides?: Partial<WebSocketComposePaneProps>): WebSocketComposePaneProps {
  return {
    isConnected: true,
    onSend: vi.fn(),
    onPing: vi.fn(),
...
describe('WebSocketComposePane', () => {
  it('renders the composer (send + ping + input)', () => {
    render(<WebSocketComposePane {...defaultProps()} />);
  ...
  it('renders nothing when hidden', () => {
    const { container } = render(<WebSocketComposePane {...defaultProps({ hidden: true })} />);
  ...
  it('sends the composed message via onSend', () => {
    const onSend = vi.fn();
    render(<WebSocketComposePane {...defaultProps({ onSend })} />);
