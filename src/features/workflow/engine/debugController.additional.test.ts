...
describe('DebugController - Additional Coverage', () => {
  ...
  it('stepNode resolves all paused threads waiting on the same node', async () => {
    const dc = new DebugController();
    const results: string[] = [];

    const p1 = dc.waitForStep('shared-node', 't1').then(() => results.push('t1'));
    const p2 = dc.waitForStep('shared-node', 't2').then(() => results.push('t2'));

    dc.stepNode('shared-node');
