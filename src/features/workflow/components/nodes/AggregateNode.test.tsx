/**
 * @vitest-environment jsdom
 */
import { describe, expect, it, vi } from 'vitest';
import { fireEvent, render, screen } from '@testing-library/react';
import AggregateNode from './AggregateNode';

vi.mock('@xyflow/react', () => ({
  Handle: (props: { type: string }) => <div data-testid={`handle-${props.type}`} />,
  Position: { Top: 'top', Bottom: 'bottom' },
}));

const handleConfigure = vi.fn();
const useNodeBaseMock = vi.hoisted(() => vi.fn(() => ({ rs: { state: 'pass' }, stateClass: '', debugStep: true, handleConfigure })));
vi.mock('./useNodeBase', () => ({ useNodeBase: (id: string) => useNodeBaseMock(id) }));
vi.mock('./NodeIcon', () => ({ NodeIcon: () => <div data-testid="node-icon" />, getNodeCategory: () => 'Data' }));
vi.mock('./NodeConfigureButton', () => ({ NodeConfigureButton: (props: { title: string; onClick: (e: React.MouseEvent) => void }) => <button data-testid="configure-btn" title={props.title} onClick={(e) => props.onClick(e as never)}>cfg</button> }));
vi.mock('./NodePausedOverlay', () => ({ NodePausedOverlay: (props: { nodeId: string }) => <div data-testid={`paused-${props.nodeId}`} /> }));
vi.mock('./NodeStatusBadge', () => ({ NodeStatusBadge: () => <div data-testid="status-badge" /> }));

describe('AggregateNode', () => {
  const baseProps = {
    id: 'agg-1',
    data: {
      label: 'Aggregate',
      mappings: [
        { id: 'm1', sourceExpression: '{{a}}', targetVariable: 'x', strategy: 'append' },
        { id: 'm2', sourceExpression: '{{b}}', targetVariable: 'y', strategy: 'append' },
        { id: 'm3', sourceExpression: '{{c}}', targetVariable: 'z', strategy: 'append' },
      ],
    } as never,
    selected: false,
    dragging: false,
    zIndex: 1,
    type: 'aggregate' as const,
    positionAbsoluteX: 0,
    positionAbsoluteY: 0,
    isConnectable: true,
    xPos: 0,
    yPos: 0,
  };

  it('renders label, count badge, preview, footer button, status, overlay, and handles', () => {
    render(<AggregateNode {...baseProps} />);
    expect(screen.getByText('Aggregate')).toBeTruthy();
    expect(screen.getByText('Data')).toBeTruthy();
    expect(screen.getByText('3 mappings')).toBeTruthy();
    expect(screen.getByText('{{a}} → x')).toBeTruthy();
    expect(screen.getByText('{{b}} → y')).toBeTruthy();
    expect(screen.getByText('+1 more')).toBeTruthy();
    expect(screen.getByTestId('status-badge')).toBeTruthy();
    expect(screen.getByTestId('paused-agg-1')).toBeTruthy();
    expect(screen.getByTestId('handle-target')).toBeTruthy();
    expect(screen.getByTestId('handle-source')).toBeTruthy();
  });

  it('falls back to default label and hides preview when there are no mappings', () => {
    render(<AggregateNode {...baseProps} data={{ mappings: [] } as never} />);
    expect(screen.getByText('Aggregate')).toBeTruthy();
    expect(screen.queryByText(/mapping/)).toBeNull();
  });

  it('shows singular mapping badge and selected class', () => {
    const { container } = render(
      <AggregateNode
        {...baseProps}
        selected={true}
        data={{ mappings: [{ id: 'm1', sourceExpression: '{{a}}', targetVariable: 'x', strategy: 'append' }] } as never}
      />,
    );
    expect(container.querySelector('.wf-node-selected')).toBeTruthy();
    expect(screen.getByText('1 mapping')).toBeTruthy();
    expect(screen.queryByText(/\+\d+ more/)).toBeNull();
  });

  it('calls configure handler from footer button', () => {
    handleConfigure.mockClear();
    render(<AggregateNode {...baseProps} />);
    fireEvent.click(screen.getByTestId('configure-btn'));
    expect(handleConfigure).toHaveBeenCalled();
  });
});
