...
import { buildExpressionFromArgValues, buildExpressionTemplate } from './expressionBuilderState';
...
export default function ExpressionBuilderView({ hints, onInsert }: Props) {
  ...
  const handleSelectFn = useCallback((fn: ExpressionFunction) => {
    ...
    setExpression(buildExpressionTemplate(fn));
  }, []);
  ...
  const handleArgChange = useCallback((idx: number, value: string) => {
    setArgValues((prev) => {
      const next = [...prev];
      next[idx] = value;
      if (selectedFn) setExpression(buildExpressionFromArgValues(selectedFn, next));
      return next;
    });
  }, [selectedFn]);
    ...
    <div className="wf-expr-builder">
      ...
      <div className="wf-expr-columns">
        ...
        <div className="wf-expr-middle">
          ...
          {/* Variable chips — click to append to the expression */}
          {hints.length > 0 && (
