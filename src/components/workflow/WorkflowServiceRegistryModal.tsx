...
import type { EnvAuthState } from '../../utils/requestAuthState';
...
export default function WorkflowServiceRegistryModal({
  ...
  return (
    <div
      className="modal-overlay wf-svc-registry-overlay"
      role="presentation"
      onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}
    >
      <div
        className="modal wf-svc-registry-modal wf-svc-fullscreen"
        role="dialog"
        aria-labelledby="wf-svc-registry-title"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="settings-header">
          <h3 id="wf-svc-registry-title">Service Registry{workflowName ? ` — ${workflowName}` : ''}</h3>
