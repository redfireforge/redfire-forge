...
import {
  createPopulateFromApiAdapter,
  type PopulateOutput,
} from './populateFromApiAdapter';
import type { DataSource, DataSourceColumn } from '../../../types';
...
const MAPPINGS: Mapping[] = [
  ...
  { id: 'pop-2', sourceId: 'api-response', sourcePath: 'email', targetPath: 'Email' },
];

// ── Adapter creation ──────────────────────────────────────

describe('createPopulateFromApiAdapter', () => {
  it('creates an adapter with correct contextId and title', () => {
    const adapter = createPopulateFromApiAdapter({ dataSource: makeDataSource() });
    expect(adapter.contextId).toBe('populate-from-api');
    expect(adapter.title).toBe('API Response → Data Source');
...
describe('serialize', () => {
  ...
  it('stringifies object values in cells', () => {
    ...
    const mappings: Mapping[] = [
      { id: 'pop-0', sourceId: 'api-response', sourcePath: 'meta', targetPath: 'Meta' },
    ];
    const adapter = createPopulateFromApiAdapter({
      dataSource: makeDataSource(),
      responseJson: response,
  ...
  it('handles null values as empty string', () => {
    ...
    const mappings: Mapping[] = [
      { id: 'pop-0', sourceId: 'api-response', sourcePath: 'name', targetPath: 'Name' },
    ];
    const adapter = createPopulateFromApiAdapter({
      dataSource: makeDataSource(),
      responseJson: response,
  ...
  it('handles root array ($) response', () => {
    ...
    const mappings: Mapping[] = [
      { id: 'pop-0', sourceId: 'api-response', sourcePath: 'vin', targetPath: 'VIN' },
    ];
    const output = adapter.serialize(mappings);
  ...
  it('resolves nested dotted source paths via getByPath', () => {
    ...
    const mappings: Mapping[] = [
      { id: 'pop-0', sourceId: 'api-response', sourcePath: 'address.city', targetPath: 'City' },
      { id: 'pop-1', sourceId: 'api-response', sourcePath: 'address.zip', targetPath: 'Zip' },
    ];
    const adapter = createPopulateFromApiAdapter({
      dataSource: makeDataSource(),
      responseJson: response,
...
describe('deserialize', () => {
  ...
  it('returns empty array for null/undefined input', () => {
    const adapter = createPopulateFromApiAdapter({ dataSource: makeDataSource() });
    expect(adapter.deserialize(null as unknown as PopulateOutput)).toEqual([]);
    expect(adapter.deserialize(undefined as unknown as PopulateOutput)).toEqual([]);
  });
  ...
  it('generates stable IDs (pop-N pattern)', () => {
    const output: PopulateOutput = {
      columns: [
    ...
    const adapter = createPopulateFromApiAdapter({ dataSource: makeDataSource() });
    const mappings = adapter.deserialize(output);
    expect(mappings[0].id).toBe('pop-0');
    expect(mappings[1].id).toBe('pop-1');
  });
});
...
describe('validate', () => {
  ...
  it('reports error for empty targetPath', () => {
    const adapter = createPopulateFromApiAdapter({ dataSource: makeDataSource() });
    const issues = adapter.validate!([
      { id: 'pop-0', sourceId: 'api-response', sourcePath: 'id', targetPath: '' },
    ]);
  ...
  it('reports error for empty sourcePath', () => {
    const adapter = createPopulateFromApiAdapter({ dataSource: makeDataSource() });
    const issues = adapter.validate!([
      { id: 'pop-0', sourceId: 'api-response', sourcePath: '', targetPath: 'Name' },
    ]);
  ...
  it('reports error for duplicate targetPath', () => {
    const adapter = createPopulateFromApiAdapter({ dataSource: makeDataSource() });
    const issues = adapter.validate!([
      { id: 'pop-0', sourceId: 'api-response', sourcePath: 'id', targetPath: 'Name' },
      { id: 'pop-1', sourceId: 'api-response', sourcePath: 'name', targetPath: 'Name' },
    ]);
  ...
  it('does not report fetch error when response JSON present', () => {
    ...
    const issues = adapter.validate!(MAPPINGS);
    expect(issues.some(i => i.severity === 'info')).toBe(false);
  });
});
...
describe('fetchSampleData', () => {
  ...
  it('stores fetched response for serialize', async () => {
    ...
    const mappings: Mapping[] = [
      { id: 'pop-0', sourceId: 'api-response', sourcePath: 'a', targetPath: 'A' },
    ];
    const output = adapter.serialize(mappings);
    expect(output.rows).toHaveLength(2);
...
describe('edge cases', () => {
  ...
  it('handles array items with missing fields gracefully', () => {
    ...
    const mappings: Mapping[] = [
      { id: 'pop-0', sourceId: 'api-response', sourcePath: 'a', targetPath: 'A' },
      { id: 'pop-1', sourceId: 'api-response', sourcePath: 'b', targetPath: 'B' },
    ];
    const adapter = createPopulateFromApiAdapter({
      dataSource: makeDataSource(),
      responseJson: response,
