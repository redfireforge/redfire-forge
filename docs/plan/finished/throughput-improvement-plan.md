...
| Tier | Phase | Description | Status | Tests |
|------|-------|-------------|--------|-------|
| **1** | PR 1 | Hot-path micro-optimizations (1O, 1B, 1H, 1N, 1J, 1D, 1E, 1L) | ✅ Complete | 13/13 scenarios validated |
| **1** | PR 2 | Transport & scheduling (1A, 1F, 1C, 1K, 1G) | ✅ Complete | 12 scenarios (pending manual validation) |
| **1** | PR 3 | Caching & deduplication (1I, 1M) | ✅ Complete | Unit tests pass |
| **1** | PR 4 | Multi-worker execution (1P) | ✅ Complete | Unit tests pass |
| **2** | 2A | Rust executor core (reqwest + tokio) | ✅ Complete | 62 Rust tests |
| **2** | 2B | Tauri commands + event streaming | ✅ Complete | 8 JS tests |
| **2** | 2C | JS integration + fallback | ✅ Complete | 68 JS tests |
| **2** | 2D | Integration tests + edge cases | ✅ Complete | 48 JS tests |
| **3A** | Sub-Group A | Types + JSONPath | ✅ Complete | 106 Rust tests |
| **3A** | Sub-Group B | Leaf helpers (deep compare, subset match, HTTP, date) | ✅ Complete | 109 Rust tests |
| **3A** | Sub-Group C | Core evaluators (field operator, assertion evaluator) | ✅ Complete | 167 Rust tests |
| **3A** | Sub-Group D | Validation engine + wiring (json_validator, validation_result) | ✅ Complete | 93 Rust tests |
| **3A** | JS Bridge | prepareRustScenario + mapRustResult passthrough | ✅ Complete | 26 JS tests |
| **3B** | — | Streaming percentiles (HDR histogram) | Not started | — |
| **3C** | — | Constant arrival rate (open model) | Not started | — |

**Total Rust tests**: 537 | **Total JS tests**: 212 | **Total test scenarios**: 43 (13 PR1 + 12 PR2 + 18 Tier 2)

**Test Scenario Documents**:
- `docs/plan/finished/throughput-pr1-test-scenarios.md` — 13/13 validated
- `docs/plan/throughput-pr2-test-scenarios.md` — 0/12 validated (pending)
- `docs/plan/tier2-rust-executor-test-scenarios.md` — 0/18 (requires Tauri desktop build)
- `docs/plan/tier3-phase3a-test-scenarios.md` — Rust unit test coverage (automated, not manual)
...
| D: Validation Engine + Wiring | `json_validator.rs`, `validation_result.rs`, executor wiring, JS bridge | — | Not started | — |

**Actual LOC so far**: 2,184 lines production + 3,614 lines tests = 5,798 total (Sub-Groups A + B + C)
