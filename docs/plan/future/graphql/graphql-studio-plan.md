...
> Re-evaluation: 4 rounds, 17 bugs fixed (a11y, design tokens, focus management). Details in git.
...
> Re-evaluation: 2 rounds, 11 bugs fixed (ARIA, click-to-navigate, CSS tokens). Details in git.
...
> Re-evaluation: 2 rounds, 19 bugs fixed (response viewer, design tokens, logic). Details in git.
...
> Re-evaluation: 5 rounds, 21 bugs fixed (auth popover, recent endpoints, CSS tokens). Details in git.
...
> Re-evaluation: 6 rounds, 33 bugs fixed (env manager, CSS full token sweep, a11y). Details in git.

### Phase 1 Gap Items — Implementation Summary (Done + Re-evaluated ×6 + Round 7–9 gap-fill)


> Re-evaluation rounds 10+: Additional audit-driven polish (10 rounds, 66 bugs across a11y, CSS tokens, UX edge cases, keyboard navigation). Details in git.
...
| 2D-3 | ✅ Update `GraphqlResponseViewer.tsx` for incremental delivery: streaming banner + chunk count badge; isStreaming state propagated per chunk; fallback to plain JSON when server doesn't honor multipart | P1 |
...
| 2D-6 | ✅ `@defer`/`@stream` + file upload mutual exclusion: blocks execution with inline error when both `formData` and incremental directives are present | P1 |
...
| # | Task | Priority |
|---|------|----------|
| 2G-1 | ✅ `GraphqlTracingView.tsx`: Apollo Tracing waterfall Gantt chart from `extensions.tracing`; horizontal bars (position = startOffset, width = duration); labels `ParentType.fieldName`; color-coded green/amber/red by duration; sortable by start time / duration / path; hover tooltip | P2 |
| 2G-2 | ✅ Query complexity estimator: pre-execution AST cost calculation (scalar +1, object +2, list × configurable multiplier, depth penalty); cost badge near Execute button (green/amber/red); configurable threshold; `shouldBlock` flag when cost > 2× threshold | P2 |
...
> **Phase 3 First-Pass Re-evaluation — 2026-06-18**: 27 gaps identified vs. Altair, Bruno, Insomnia, Hoppscotch, Apollo Sandbox, Postman. New tasks marked **[R3-new]**. See §3.3 feature spec for rationale.
>
> **Phase 3 Second-Pass Technical Review — 2026-06-18**: 14 correctness issues fixed. 9 missing tasks added. (`fflate` dependency added in this pass, subsequently removed when 3C was cut.)
>
> **Phase 3 Fourth-Pass Architecture Review — 2026-06-18**: 11 critical implementation-blocker gaps fixed: (1) complete UI layout spec added — History/Collections/Mock panels anchor to a new inner `GraphqlStudioActivityBar`; Code Gen opens as right drawer; Runner as bottom panel tab; (2) `lastExecutedAt` field added to `GraphqlCollectionItem` (drives "Recent" section); (3) `CollectionRunEvent.error` given a structured `{ phase, message }` shape; (4) import Replace mode now explicitly requires a single IDB transaction for atomicity; (5) history response storage capped at 512KB with truncation banner; (6) mock config persistence specified — reload from `localStorage` on proxy restart, re-sync but don't auto-enable; (7) APQ+batch interaction documented as mutually exclusive; (8) APQ GET encoding detailed (`encodeURIComponent(JSON.stringify(...))`, proxy decoding spec); (9) `detectDeprecationChanges` given complete implementable algorithm using `graphql` package's `isObjectType`/`isInterfaceType`/`getTypeMap()`; (10) code gen subscription support matrix added — `typescript-urql/apollo/python-gql` support subscriptions; others emit warning comments; (11) `rf.test()` `pendingTests` clarified as internal closure (not on public `RfContext`); `rf.getCollectionVar` outside collection context → silent no-op via null `collectionVars` param; `djb2` reference implementation added; snapshot-vs-snapshot diff acknowledgements explicitly NOT persisted. New tasks: `GraphqlStudioActivityBar` (3A-15), variables JSON validation at save time (3A-16). `detectDeprecationChanges` algorithm fully specified inline.
>
> **Phase 3 Third-Pass Deep Review — 2026-06-18**: 7 critical bugs fixed: (1) static `CriticalityLevel` import defeated lazy loading — moved inside `await import()`; (2) dedup key `djb2(query+vars)` missing `connectionId` — identical queries to two different connections would be deduplicated together; (3) history `id = connectionId+timestamp` has millisecond-collision risk under parallel execution — changed to `randomUUID()`; (4) IDB `graphql-history` needs compound `[connectionId, timestamp]` index for efficient per-connection range queries; (5) `DEPRECATED` detection via `c.type.includes('DEPRECATED')` would never match (change type values don't contain that word) — replaced with explicit `@deprecated` directive schema traversal; (6) `rf.test()` async fn cannot be `await`ed inline — added promise-collection pattern (`pendingTests[]` array, `Promise.allSettled` after script body); (7) `rf.store` in non-runner context was undefined — specified no-op stub. 5 high-priority gaps fixed: export `sortOrder` in JSON example + import fallback documented; `typescript-react-query` now shows complete output (fetcher + hook); unit test tasks added for 3A (3A-14), 3B (3B-11), 3D (3D-10), 3E (3E-14); code gen CSS task added (3C-12).
>
> **Phase 3 Fifth-Pass Competitive Re-evaluation — 2026-06-18**: Thorough re-evaluation against updated competitive landscape (June 2026). Researched: Altair v8.5.0, Bruno v3.4.2, Hoppscotch 2026.5.0, Insomnia (Kong), Postman (v11+), Apollo Sandbox/Studio, GraphQL Code Generator v6, `@graphql-inspector/core` v7.1.3, MSW, Mockd, Mockpit, Moquerie, ariadne-codegen. **8 issues identified and addressed below:**
>
> **R5-1 — `@graphql-inspector/core` version pin outdated (§3.3 deps + 3D)**: Plan pins `^5.x` but latest is `v7.1.3` (Apr 2026). Breaking change in v7: `diff()` is now async (since v3+, already handled in our `computeSchemaDiff`). New in v7.1.3: `INPUT_FIELD_ADDED` with default value is now `Dangerous` (was `NonBreaking`) — important for rolling deploys. **Fix**: Update dependency pin from `^5.x` to `^7.x` in Phase 3 deps and add a note about the `INPUT_FIELD_ADDED` reclassification.
>
> **R5-2 — GraphQL Codegen v6 client-preset not addressed in code gen decision (§3C)**: The plan explicitly rejects `@graphql-codegen` dependency (correct for bundle size), but does not acknowledge that Codegen v6 (Apr 2026) generates only used types (no full schema dump). This strengthens the plan's built-in AST walker approach — we already do per-operation type gen, which is what Codegen v6 now does. **Fix**: Add a note acknowledging Codegen v6 parity and confirming the built-in approach remains correct.
>
> **R5-3 — Postman GraphQL Collection Runner still not supported (§3A-8/3A-9)**: Postman's native GraphQL client still cannot run collections in the Runner (open issue #12196, last updated Aug 2025). Users must send GraphQL as POST requests to use the Runner. **This is a significant competitive advantage for RedfireForge** — our Collection Runner (3A-8/3A-9) natively supports GraphQL operations. **Fix**: Add competitive note to 3A.
>
> **R5-4 — Altair's script execution order differs from plan (§3B)**: Altair runs collection pre-request → parent collections' pre-request → window pre-request (bottom-up collection hierarchy). The plan specifies Global → Collection → Item (top-down). Both are valid, but the plan should document this difference explicitly so users migrating from Altair understand the inversion. **Fix**: Add migration note to 3B.
>
> **R5-5 — Bruno v3 has collection runner + code generation (§3A/§3C)**: Bruno v3.4.2 now has a full collection runner (run entire folders with iterations) and code generation (curl, JS, Python, etc. from request menu, including for GraphQL). The plan correctly competes but should update the competitive context to acknowledge Bruno's advancements. **Fix**: Update competitive context notes in 3A and 3C.
>
> **R5-6 — Insomnia v12 has AI-generated mock servers (§3E)**: Insomnia now offers cloud and self-hosted mock servers with AI-generated responses from natural language prompts. However, these are generic HTTP mocks, not schema-aware GraphQL mocks with resolver-level control. RedfireForge's SDL-driven mock server with per-field resolvers, scenarios, and scalar factories remains a stronger GraphQL-specific offering. **Fix**: Update competitive context note in 3E.
>
> **R5-7 — Missing `rf.expect()` Chai-style assertions (§3B)**: Postman uses Chai BDD syntax (`pm.expect(value).to.equal(...)`) which is more expressive than our `rf.assert(condition, msg)`. **Resolution**: Added as task 3B-12 in R5, then **deferred** in the scope reduction pass — `rf.assert()` is sufficient for a performance workbench; `rf.expect()` is a Postman migration convenience, not a core testing need. Reopen if users migrate from Postman collections at scale.
>
> **R5-8 — No Postman/Insomnia/Hoppscotch collection import (§3A-5)**: The plan only exports/imports RedfireForge's own `_exportMeta v1.1` format. **Resolution**: Added as task 3A-17 in R5, then **deferred** in the scope reduction pass alongside the code gen cut. The native export/import format covers the primary workflow; migration import is a nice-to-have.
>
> **Scope reduction decision (2026-06-18)**: Entire **Phase 3C (Code Generation)** removed — 12 tasks, ~1200 LOC, `fflate` dependency, `GraphqlCodeGenPanel.tsx`. Code gen targets developers *building* GraphQL-consuming applications, not developers *testing and performance-testing* them. "Copy as cURL" (already in 3A-2 context menu) is the only testing-relevant snippet and remains. Additionally: 3B-9 (global scripts), 3B-12 (rf.expect), and 3A-17 (Postman import) deferred. Revised scope: ~26 files, ~5500 LOC.
...
> **Condensed** — 4 rounds, 17 bugs fixed (a11y, design tokens, focus management, dead code). Full details in git history.
...
> **Condensed** — 19 rounds, 150+ bugs fixed across all Phase 1 files (CSS tokens, a11y, type safety, dead code, UX polish). Full details in git history.

| Round | Bugs | Focus |
|-------|------|-------|
| 1 | 9 | Code quality, ARIA structure, CSS tokens |
| 2 | 3 | Component architecture, type imports |
| 3 | 1 | Keyboard navigation edge case |
| 4 | 2 | Popover focus trap, polling state |
| 5 (CSS) | 33 | Full CSS token overhaul |
| 6–9 | 40 | Response viewer, schema explorer, connection bar |
| 10–15 | 44 | Auth popover, env manager, edge cases |
| 16–19 | 7 | Final polish, polling config, StrictMode |
...
### 23.2 Competitive Landscape Summary (June 2026)

> Full comparison tables in git history. Key findings:

**Subscriptions**: Hoppscotch leads but lacks assertions/stats. RedfireForge is **best-in-class** with JSONPath assertions + msg/sec stats. Protocol auto-detection is unique.

**Visual Query Builder**: Bruno leads with bidirectional sync but no fragments/directives (max 7 levels). RedfireForge MVP shipped (3-column, one-way sync); advanced features deferred.

**Incremental Delivery** (`@defer`/`@stream`): **No free tool** supports this with good UX. Spec still unstable (alpha.9). RedfireForge uses `meros` with version-aware parser.

**File Upload**: Altair is the reference. RedfireForge adds drag-drop, progress, size validation. `graphql-multipart-request-spec` is stable.

**Performance**: Apollo Tracing format is deprecated but widely emitted. RedfireForge supports it + client-side complexity estimation (unique differentiator). OpenTelemetry integration is a Phase 3+ consideration.
...
### 23.6 Task-by-Task Evaluation (Condensed)

> Full per-task evaluation in git history. Summary of completed (✅) and deferred (⏸) items:

**2A — WebSocket Subscriptions (9 tasks)**: ✅ All P0/P1 done (Sprints 1–2, 5). WS proxy (2A-1 stub), modern WS (2A-2), legacy WS (2A-3 vendored ~200 LOC), auto-detection (2A-4 deferred — close-code fallback), state machine (2A-5), reconnect (2A-6), status pill (2A-7), auth (2A-8), URL derivation (2A-9).

**2B — SSE Subscriptions (4 tasks)**: ✅ All done (Sprint 3). `graphql-sse` client (2B-2), auto-detection heuristic (2B-4), transport selector dropdown.

**2C — Subscription UI (5 tasks)**: ✅ 2C-1 through 2C-5 all done (Sprints 2, 4, 8). Log with stats/toolbar/filter/export, JSONPath filter, assertion badges.

**2D — Incremental Delivery (5 tasks)**: ✅ 2D-1/2/3/5 done (Sprint 7). `meros` multipart parser, proxy streaming, response viewer with chunk tracking, `hasIncrementalDirective()` utility. ⏸ 2D-4 (chunk tracker toolbar) deferred.

**2E — File Upload (5 tasks)**: ✅ 2E-1/2/3/4/5 done (Sprints 4, 8). Drag-drop UI, multipart FormData builder, busboy proxy, XHR upload progress, client-side size validation.

**2F — Visual Query Builder (11 tasks)**: ✅ MVP done (Sprint 6): 2F-1 through 2F-5 + 2F-9. ⏸ Deferred to post-2.1: 2F-6 (fragments), 2F-7 (directives), 2F-8 (aliases), 2F-10 (union/interface), 2F-11 (persistence).

**2G — Performance & Tracing (5 tasks)**: ✅ 2G-1/2 done (Sprint 7). Apollo Tracing waterfall, query complexity estimator. ⏸ Deferred: 2G-3 (response time histogram), 2G-4 (complexity config UI), 2G-5 (histogram query detection).
...
### 23.8–23.10 Phase 2 Risk, Effort, & Sprint Order (Condensed)

> Detailed risk tables, LOC estimates, and sprint tree in git history. All sprints now complete.

**Total actual effort**: ~5150 LOC across 27 files, 8 sprints, 1015 tests.
**Sprint order**: S1 (foundation) → S2 (WS subs) → S3 (SSE) → S4 (file upload + filter) → S5 (legacy compat + migration) → S6 (query builder) → S7 (incremental delivery + tracing) → S8 (assertions + upload progress).
...
### 23.13–23.15 Phase 2 Pre-Implementation Evaluation Rounds 2–4

> **Condensed** — 3 rounds totaling 59 gaps identified, all addressed in Sprints 1–6.
> Key outcomes preserved below; verbose gap-by-gap details in git history.

**Round 2 (§23.13)** — 28 gaps: transport model mismatch (Phase 1 HTTP-only vs Phase 2 polymorphic), missing shared types, selector gaps, server route architecture, Docker test server, Sprint plan tree.

**Round 3 (§23.14)** — 15 gaps: StrictMode tab persistence bug (fixed), `localStorage` → `storage.ts` migration needed, `subscriptions-transport-ws` legacy decision (Option C: hybrid), Docker server already exists in `docker/websocket/graphql/`, vite proxy config gaps.

**Round 4 (§23.15)** — 16 gaps + 5 reuse opportunities: WS Studio message log reuse, SSE Studio connection pattern reuse, auth resolve bridge, metrics hook pattern, server proxy pattern. Sprint plan tree finalized (5 sprints for Phase 2.0).

**Architecture Decision**: Option C — Hybrid transport (HTTP for query/mutation, direct WS for browser subscriptions, proxy WS for auth/TLS, direct SSE for `/stream` endpoints, proxy SSE for auth).

**Sprint Plan** (from §23.15):
```
Sprint 1: Foundation (graphqlClient.ts, server stubs, auth bridge, types/selectors)
Sprint 2: WebSocket Subscriptions (real transport, state machine, subscription log UI)
Sprint 3: SSE Transport (graphql-sse, transport selector, auto-detection)
Sprint 4: File Upload + Filter Bar (multipart spec, drag-drop, busboy proxy, JSONPath filter)
Sprint 5: Legacy Compat + Testing (subscriptions-transport-ws, storage migration, >90% coverage)
```

**Pre-Sprint Checklist** (all completed in Sprint 1):
- [x] 2-PRE-1: Architecture decision recorded
- [x] 2-PRE-2: `graphqlClient.ts` transport interface + HTTP impl
- [x] 2-PRE-4: Server route stubs (`/api/graphql/subscribe`, `/sse`, `/upload`)
- [x] 2-PRE-6: npm packages added (`graphql-ws`, `graphql-sse`)
- [x] 2-PRE-7: `localStorage` → `storage.ts` migration
- [x] 2-TYPE-1: Phase 2 types added to `graphql.ts`
- [x] 2-SEL-1: Phase 2 selectors added to `selectors.ts`
- [x] 2-E2E-1: E2E fixture scaffolded

---

### Sprint 1 — Foundation (Condensed)

> `graphqlClient.ts` (transport interface + HTTP impl), `buildConnectionParams` auth bridge, `deriveWsEndpoint`, server route stubs, Phase 2 npm packages.
> **Architecture**: Option C — Hybrid transport (HTTP for Q/M, direct WS for subscriptions, proxy WS for auth/TLS, direct/proxy SSE).
> **6 bugs fixed across 4 rounds**, 12 new tests (gqlFetch.test.ts). 990 tests pass.

---

### Sprint 2 — WebSocket Subscriptions (Condensed)

> Real `graphql-transport-ws` transport, subscription state machine, message ring buffer (5000), rolling stats, `useSubscriptionOrchestration` extracted, live subscription log UI with stats/toolbar/filter/expand.
> New tests: 122 (useGraphqlSubscription: 29, GraphqlSubscriptionLog: 29, graphqlClient: 64).
> **26 bugs fixed across 5 re-evaluation rounds**. 445+ tests pass.

---

### Sprint 3 — SSE Subscription Transport (Condensed)

> SSE transport via `graphql-sse`, per-tab transport selector dropdown, auto-detection (`/stream` → SSE).
> **Key decisions**: SSE auth via HTTP headers; distinct-connections mode; transport change does NOT mark tab unsaved; dropdown disabled during active subscription.
> **12 bugs fixed across 4 rounds**, 40+ new tests. Details in git history.

---

### Sprint 4 — File Upload + JSONPath Filter (Condensed)

> File Upload (`GraphqlFileUpload.tsx`, `multipartBuilder.ts`, busboy proxy) + JSONPath subscription filter mode.
> New files: `GraphqlFileUpload.tsx/test`, `multipartBuilder.ts/test`. Deferred: upload progress (→Sprint 8).
> **20 bugs fixed across 8 rounds**, 73+ new tests. Details in git history.

---

### Sprint 5 — Legacy WS Compat + Storage Migration (Condensed)

> Legacy `graphql-ws` subprotocol (~200 LOC vendored client), `localStorage` → `storage.ts` migration, >90% coverage achieved.
> Key fixes: `position: fixed` popovers (overflow bug), Variables panel height collapse, Prettify → Monaco model sync.
> Deferred: shared protocol-log extraction (2-SHARED-1 → future sprint).
> **20 bugs fixed across 4 rounds**, 5 new tests. Details in git history.

---

### Sprint 6 — Visual Query Builder (Condensed)

> 3-column full-width builder: `useGraphqlQueryBuilder.ts` (useReducer) + `queryBuilderGenerator.ts` (SDL gen) + `GraphqlQueryBuilder.tsx` (field tree, arg inputs, search, "Edit in Editor").
> New files: `useGraphqlQueryBuilder.ts/test`, `queryBuilderGenerator.ts/test`, `GraphqlQueryBuilder.tsx`.
> **12 bugs fixed across 4 rounds**. 970 tests pass. Details in git history.

### Sprint 7–8 Re-evaluation (Condensed)

> Sprint 7 (incremental delivery + tracing + complexity): 1 bug — `ApolloTracingData.parsing`/`validation` made optional.
> Sprint 8 (subscription assertions + upload progress): 2 bugs — XHR upload progress zero test coverage (+9 tests), Content-Type case-insensitive deletion.

### Phase 2 Full Cross-Sprint Re-evaluation (3 Rounds, 12 Bugs, 10 New Tests)

> **Scope**: Holistic review of Sprint 1–8 integrations across 6 surfaces: orchestrator, transport, execution, subscriptions, server proxy, shared types.

**Round 1** — 9 bugs (2 Critical, 4 High, 3 Medium):
- `useGraphqlExecution.ts`: abort controller ownership guard (6 sites)
- `useGraphqlSubscription.ts`: unmount cleanup, `clear()` stats reset, session guard on callbacks
- `GraphqlStudioPage.tsx`: subscription tab-switch disconnect, fileEntries cleanup on tab close
- `graphqlClient.ts`: legacy WS `connection_error` handler
- `graphql.ts`: `ApolloTracingData.execution` made optional
- `useGraphqlExecution.ts`: empty incremental stream error handling

**Round 2** — 3 test coverage gaps: `clear()` stats assertion, session guard tests (×3), legacy WS `connection_error` tests (×3).

**Round 3** — 3 bugs: API Key default `headerName` not persisted, environment load race condition (functional updater), complexity estimator summing all operations (added `operationName` filter).

#### Deferred Items (Feature Gaps for Future Sprints)

| Area | Deferred To |
|------|-------------|
| WS proxy not wired (`requiresWsProxy()` unused) | Sprint 9+ |
| Close-code auto-fallback (4406/4400 → `graphql-ws`) | Sprint 9+ |
| Tab headers ignored for WS (only `connectionParams`) | Sprint 9+ |
| Per-tab HTTP execution (page-global hook) | Architectural refactor |
| Builder + connection bar sync | Sprint 6 polish |
| `avgLatencyMs` never computed | Sprint 9+ |
| Server proxy security (SSRF, timeouts) | Sprint 9+ |
...
| **Total Phase 2 tests** | **1015** (989 client + 26 server) |
