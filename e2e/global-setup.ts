...
export default async function globalSetup(): Promise<void> {
  ...
  if (!fullDocker && !graphqlOnly && !grpcOnly && !gql5Docker) {
    console.log(
      '[global-setup] Skipping Docker startup (set E2E_WITH_DOCKER=1, E2E_GRAPHQL_SERVER=1, E2E_GRPC_SERVER=1, or E2E_GQL5_DOCKER=1)',
    );
    return;
  }
